...
import { Bot, Brain, Trash2, MessagesSquare, ChevronDown, Check } from "lucide-react";
...
export function ChatHeader({
  ...
  return (
    <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-md rounded-b-3xl sm:rounded-3xl p-3 sm:p-5 mb-2 sm:mb-4 border-b sm:border border-slate-200/50 dark:border-slate-700/50 shadow-sm flex flex-col sm:flex-row items-center justify-between shrink-0 z-20 sticky top-0 gap-3">
      <div className="flex items-center justify-between w-full sm:w-auto gap-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 sm:w-12 sm:h-12 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/20 relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:animate-[shimmer_2s_infinite]" />
            <currentMode.icon className="w-4 h-4 sm:w-6 sm:h-6 text-white relative z-10" />
          </div>
          <div className="flex flex-col">
            <div className="relative">
              <button
                onClick={() => setIsMobileDropdownOpen(!isDropdownOpen)}
                className="flex items-center gap-2 text-base sm:text-xl font-black text-slate-900 dark:text-white tracking-tight hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors group"
              >
                ...
                <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : ""} text-cyan-500`} />
              </button>
        ...
        {mode === "student_agent" && (
          <div className="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            <select
              ...
              className="px-2 sm:px-3 py-1.5 text-[10px] sm:text-xs font-bold rounded-lg sm:rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 flex-1 sm:flex-none min-w-[100px]"
            >
            ...
            <select
              ...
              className="px-2 sm:px-3 py-1.5 text-[10px] sm:text-xs font-bold rounded-lg sm:rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 disabled:opacity-60 flex-1 sm:flex-none min-w-[100px]"
            >
              <option value="">
                {studentQuizzesLoading
                  ? t("loadingExams")
