...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (lockoutTime > 0) {
      setError(`تم تعليق المحاولات. انتظر ${Math.ceil(lockoutTime / 1000)} ثانية`);
      return;
    }
    setLoading(true);
    setError("");
    try {
      ...
      localStorage.setItem("login_attempts", JSON.stringify({ count: newAttempts, timestamp: Date.now() }));
      if (newAttempts >= 20) {
        setLockoutTime(lockoutDuration);
        setError(`تم تعليق المحاولات بسبب محاولات فاشلة متكررة. انتظر ${Math.ceil(lockoutDuration / 1000)} ثانية`);
      } else {
    ...
    <div className="max-w-md mx-auto py-10">
      <div className="modern-card p-8 sm:p-10">
        ...
        {(error || success || resetMessage) && (
          <div className={`mb-8 p-4 rounded-2xl text-sm font-bold animate-in fade-in slide-in-from-top-2 duration-300 ${
            error ? "bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 text-rose-600 dark:text-rose-400"
        ...
        {showForgotPasswordForm ? (
          <form onSubmit={handleForgotPassword} className="space-y-6">
            <div>
              <label htmlFor="forgot-email" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 mr-1">البريد الإلكتروني</label>
              <input
                id="forgot-email"
                type="email"
                required
            ...
            <div className="text-center mt-6">
              <button
                type="button"
                onClick={() => { setError(""); setResetMessage(""); setShowForgotPasswordForm(false); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="login-email" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 mr-1">البريد الإلكتروني</label>
              <input
                id="login-email"
                type="email"
                required
            ...
            <div>
              <label htmlFor="login-password" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 mr-1">كلمة المرور</label>
            ...
            <div className="flex justify-end">
              <button
                type="button"
                onClick={() => { setError(""); setResetMessage(""); setShowForgotPasswordForm(true); }}
        ...
        <div className="mt-8">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white dark:bg-brand-navy text-slate-400 font-bold">أو</span>
            </div>
          </div>
        </div>
