/**
 * Supabase database types — moved from `apps/web/src/types/database.ts`
 * in Spec 004 Phase 2 (T011). The original file is preserved as a
 * thin re-export from `apps/web/src/types/database.ts` so existing
 * imports continue to work.
 *
 * The shape is hand-maintained today; a future migration will replace
 * this with the auto-generated types from `supabase gen types
 * typescript --linked` once the monorepo's type-generation step is
 * added (tracked in plan.md §Cross-cutting concerns).
 */
export type Json =
  | string
  | number
