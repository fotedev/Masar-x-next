...
- [ ] T015 [P] Add CI step that runs `gitleaks detect --no-git --source <each-built-artifact>` against every built app bundle (Electron asar, Expo bundle, Next.js static export); build fails on any match. The CI job that runs gitleaks MUST also be registered as a **required status check** on the protected branch (same enforcement pattern as T014) — a CI job that runs but isn't required is observability, not enforcement. The registration of the check as "required" is a one-time GitHub repository setting (Branch protection rules → Require status checks to pass before merging), performed manually via the GitHub UI or via a repo-config-as-code tool (e.g., Terraform, `gh api`). This task's deliverable is BOTH the CI step AND the branch-protection registration, not just one or the other.
...
- [x] T025 [US1] Smoke-test on at least one target platform; verify the installer does not trigger OS security warnings (spec FR-001 / SC-001) — **PASS** (2026-08-21): T020.2 standalone integration + `ELECTRON_RUN_AS_NODE=1` fork-bomb fix + `dereference-standalone.cjs` postbuild script. NSIS installer (`Masar X-0.5.6-x64.exe`, 184 MB) builds cleanly. Packaged app spawns server.js, HTTP 200 on the Arabic home page, SQLite `cache.db` magic bytes verified, graceful shutdown with 0 zombie processes. 21/28 vitest pass (7 failures are the known `better-sqlite3` ABI mismatch on Windows + Node 24, deferred to CI Linux). DPAPI `session.bin` and Menu bar UI verification deferred to manual interactive session — see `smoke-test-results.md`.
...
**Independent Test**: Build and install the mobile app on a real iOS device and a real Android device (or iOS Simulator + Android Emulator). Verify the following FRs end-to-end: FR-002 (auth continuity — same user signs in and sees the same data within 5s), FR-M02 (touch-optimized quiz completion), FR-M03 (offline read — kill network mid-session, content remains accessible from the local read cache per T033), FR-009 (PDF upload via `expo-document-picker` per T034), FR-012 (native share sheet invocation per T035), and FR-008 (RTL/LTR switching per device language, including `dir` attribute assertion per T026).
...
- [x] T054 [US5] PARTIAL — `supabase/functions/ai-chat/index.ts` implemented in PR #26 for the non-streaming path: receives the `AiRequest` from the shared package, reads the AI provider key from its own environment, calls Gemini, returns the contract's `AiResponse`, applies the existing rate limits via the Next.js proxy at `/api/ai-chat`. The contract's `streamAiMessage` is NOT YET IMPLEMENTED — the function returns JSON only, so the shared client's streaming variant silently produces no output for v1. See T054a.
- [ ] T054a [US5] Follow-up: add SSE streaming support to `supabase/functions/ai-chat/index.ts` so the contract's `streamAiMessage` works end-to-end. Use Gemini's `generateContentStream`, return `text/event-stream` content type, detect via `Accept: text/event-stream` request header. The shared client's SSE parser in `packages/shared/src/ai/index.ts` is already in place and ready to consume.
...
**Independent Test**: Measure the following against documented baselines (captured before any monorepo work landed): desktop cold-start (per spec SC-007 and T058, ≤2s on the documented reference machine), mobile list scroll (per spec SC-008 and T059, no visible jank during 500-item scroll on the documented mid-range Android device, FPS floor captured), mobile data usage for a 10-minute study session (per T060, ≤50% of the equivalent mobile-browser session), and web app Lighthouse score (per spec SC-009 and T057, no regression >5 points). All four measurements use the tooling and reference devices documented in T057–T060; no visual-inspection "feels fine" claims.
