...
| G6.3 | Lighthouse cadence unverified | **Verified 2026-09-18 — it was broken:** every push-triggered run failed at parse time (zero jobs, no log) because an unquoted colon-space in a step name (`name: Assert category thresholds (SC-009: …)`) made GitHub reject the whole workflow file; fixed by quoting (`0b3…`, see changelog). Workflow is record-only until the owner sets `LHCI_BASELINE_*` repo variables (one-time setup documented in the file header) | 🟡 Medium | partial | run history + YAML parse check 2026-09-18 | `.github/workflows/lighthouse.yml` | Owner: set the two baseline variables after the first green run; then the >5-point budget (SC-009) becomes enforceable |
