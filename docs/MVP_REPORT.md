...
| --- | --- | --- | --- | --- | --- | --- | --- |
| G1.1 | Production content entry | The study flow renders real data only if subjects/lectures/summaries/quizzes are entered. Code paths exist; actual content population is the last owner-declared MVP task. **2026-09-17 dependency found:** env sweep shows `NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME`/`_UPLOAD_PRESET` missing in Vercel prod (and nowhere locally) — the Cloudinary upload/image path cannot work in prod until the owner adds them or declares Cloudinary retired for MVP (Supabase Storage route) | 🔴 Critical | partial | routes + hooks read 2026-09-16; prod env gap confirmed 2026-09-17 (see `specs/009.../checklists/env-sweep.md`) | `apps/web/src/app/[locale]/admin-dashboard/`, `[locale]/add-file/`, `[locale]/add-video/`, `[locale]/edit-summary/`, `apps/web/src/hooks/useContentForm.ts` | Owner decides Cloudinary vs Supabase Storage (env-sweep checklist lists exact `vercel env add` commands) → then E2E admin upload smoke → populate launch cohorts |
...
| ID | Item | Description | Priority | State | Verified | File/Module location | Suggested fix / action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| G2.1 | Production AI smoke | **Env-level answer found 2026-09-17 (2e4520d):** there is no `ai_chat_disabled` env — the gate is `AI_GATEWAY_API_KEY` (`api/ai/chat/route.ts:61`), and it is absent from Vercel prod ⇒ server chat returns 503 and clients use the **Puter.js fallback** (consistent with the zane client-side architecture). Remaining: owner confirms intent + one browser smoke round-trip | 🟠 High | partial | env sweep 2026-09-17 (`specs/009.../checklists/env-sweep.md`); browser smoke pending | `apps/web/src/app/api/ai/chat/route.ts:61`, Vercel env | Owner: confirm Puter-only is intended for launch (or provision `AI_GATEWAY_API_KEY`); then one logged-in chat + one rate-limit rejection check |
...
| ID | Item | Description | Priority | State | Verified | File/Module location | Suggested fix / action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| G6.1 | No e2e coverage of the core study flow | **DONE 2026-09-17 (`c245170` + `08a0a65`)**: Playwright harness + ar/en public happy-path spec (home → subjects → subject detail, data-dependent steps soft-skip without backend data) — local run 5 passed / 1 data-skip; CI `e2e` job wired in `ci.yml` (build with public Supabase repo-Variables → `next start` → test → artifacts). Repo Variables set. **Pending owner:** add `e2e` to required checks (ruleset API token got 404; instructions in `RULESET.md`) | 🟠 High | done | local run + CI wiring 2026-09-17; CI green-run pending first PR | `apps/web/playwright.config.ts`, `apps/web/e2e/study-flow.spec.ts`, `.github/workflows/ci.yml` (`e2e` job) | First PR confirms the job green end-to-end; owner adds required check |
...
## 8. Top critical blockers (ranked by impact) — updated pass 3 (2026-09-17)

Pass-3 status: G3.5 ✅ done, G3.1 ✅ done, G6.1 ✅ done (CI job live), G3.6 config ✅ landed. The launch-critical list is now:

1. **G1.1 — Production content entry, now gated on the owner's Cloudinary decision.** The env sweep found the Cloudinary public vars missing in prod (and no local source of truth) — the upload/image path cannot work in prod until the owner adds the two vars (`vercel env add …`, exact commands in the env-sweep checklist) or declares Cloudinary retired for MVP in favor of Supabase Storage. Everything downstream (admin upload smoke → content population → launch) waits on this one decision.
2. **G3.4 — Admin role-propagation test (owner-assisted).** Runbook ready; execute and record.
3. **G6.1 residue — first CI green run + required-check promotion.** The `e2e` job is wired but hasn't run on a PR yet; owner adds it to required checks after first report (RULESET.md has the 2-minute path).

*(Post-MVP queue unchanged: G7.3 release drill, G3.7 signing, G5.1 Electron bump, G3.6 alert triage, G3.9 migration reconciliation — the last one new from the audit.)*
