...
| `masarx-lockup.svg` | علامة + wordmark (lockup رأسي — أرشيفي، لم يعد مستخدمًا في الإنتاج) |
...
**أصول الإنتاج الفعلية — `apps/web/public/` (2026-09-23):**

| الملف | الوصف |
|---|---|
| `logo_lockup.svg` | **الـ lockup الأفقي الرسمي** (العلامة + wordmark «MASARX» أفقيًا، نسبة 2.895:1) — مُركَّب من الماسترات بتعديل transforms فقط؛ الفجوة الأفقية 48.31 وحدة = فجوة الـ lockup الرأسي الرسمي |
| `masarx-mark.svg` | نسخة حرفية من `masarx-mark.svg` الماستر للسياقات المربعة |
...
3. **قاعدة الـ wordmark العالمية (معدّلة 2026-09-23 بقرار المالك — ألغت قاعدة «العربية = علامة فقط»):** الـ **lockup الأفقي** (`logo_lockup.svg`: العلامة + wordmark «MASARX» اللاتيني) هو الشكل الرسمي لشريط التنقل في **اللغتين** — عالمي مثل Google/Stripe/Linear، ويُقرأ بالاتجاه LTR داخل صف RTL دون انعكاس. **العلامة المستقلة** (`masarx-mark.svg` / `getLogoPath`) للسياقات المربعة والأيقونية: الفوتر ودرج الموبايل (حيث يوجد نص براند مجاور)، بلاطات الدخول/التسجيل، أيقونات الإشعارات. الـ wordmark اللاتيني معتمد عالميًا كمعرّف براند؛ يبقى تصميم lockup عربي مطبعي مرحلة قادمة اختيارية. `logo_AR.webp` / `logo_EN.webp` أصول قديمة (legacy — يُستبدل استهلاكها تدريجيًا).
